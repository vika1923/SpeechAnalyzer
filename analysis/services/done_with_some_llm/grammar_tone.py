...
def send_gpt4o_request(prompt, text, temperature=0.3, max_tokens=500):
    ...
    logger.info(
        f"Sending request to GPT-4o-mini with prompt: {prompt[:50]}..., text: {text[:50]}..., temperature: {temperature}, max_tokens: {max_tokens}"
    )
...
def send_gpt5nano_request(prompt, text, max_completion_tokens=500):
    ...
    logger.info(
        f"Sending request to GPT-5-nano with prompt: {prompt[:50]}..., text: {text[:50]}..., max_completion_tokens: {max_completion_tokens}"
    )
...
def send_api_request(
    prompt, text, model=default_model, temperature=0.3, max_tokens=16000
):
...
def get_ielts(text, use_nano=True, max_tokens=10000) -> Optional[str]:
    prompt = """You are an IELTS and CEFR scorer.
...
def fix_punctuation_and_paragraphs(
    text, use_nano=True, max_tokens=4000
) -> Optional[str]:
    prompt = """You are a professional text editor. 
...
def fix_grammar(text, use_nano=False, max_tokens=4000) -> Optional[str]:
    prompt = """You are a professional public speaking assessor. You will be given a part of a public speech transcript. Your task is to:
...
def get_mistakes_and_text(text_to_check):
    ...
    lines_to_process = (
        lines[:corrected_text_start_idx] if corrected_text_start_idx != -1 else lines
    )
        ...
        if '"' in line and "should be" in line:
            ...
            if idx != -1:
                ...
                corrected_text = (
                    corrected_text[:idx]
                    + correct_phrase
                    + corrected_text[idx + len(incorrect_phrase) :]
                )
